...
We use the most popular, performing and deployment friendly architecture backend developers use to construct and deploy their micro-service. 
1. The entire Picoflow application is sitting on NestJS/NodeJS/Docker. It can be deployed to any cloud or self-hosted.
2. We use NOSQL  MongDB/DocumentDB/CosmoDB as session database.
3. Consuming client will call the same endpoint /run but with different flow parameters and get back a customizable response.
...
> We also import the configuration module provided by NestJS to configuration to read values from .env or environmental variables.
...
We have done the routine setup. You can vary from the standard prescription above if you are already a NestJS developer.
We can _now_ focus on the main code that illustrates the concepts and the advantages of using Picoflow.
